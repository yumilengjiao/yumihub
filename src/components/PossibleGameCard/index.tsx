...
import usePossibleGameStore, { PossibleGameInfo } from '@/store/possibleGamesStore';
...
const PossibleGameCard: React.FC<{ data: PossibleGameInfo }> = ({ data }) => {
  ...
  const displayData = useMemo(() => {
    ...
    // 3. Ymgal (月幕) 数据
    if (activeSource === 'ymgal' && data.ymgal?.data?.result?.[0]) {
      const item = data.ymgal.data.result[0];
      return {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row w-full h-full">
        ...
        {/* 右侧：文字与交互区 (占据剩余全部宽度) */}
        <div className={`
          flex flex-col grow min-w-0 transition-all duration-300
          ${isExpanded ? 'ml-6' : 'ml-4 justify-center pr-8'}
        `}>

          {/* 标题 */}
          <h3 className={`
            font-black text-zinc-100 truncate transition-all duration-300
            ...
            <div className="flex flex-col flex-grow min-h-0 animate-in fade-in duration-500">
              ...
              <div className="mt-auto pt-4 flex items-center justify-between border-t border-zinc-800">
                <div className="flex gap-2">
                  {(['vndb', 'bangumi', 'ymgal'] as const).map((source) => (
                    <button
                      key={source}

  ...
  return (
    <div className={cn("flex items-center justify-between gap-8 p-6 rounded-[24px] hover:bg-zinc-50 transition-all", className)}>
      <div className="flex flex-col space-y-1">
        <Label className="text-2xl font-bold tracking-tight text-zinc-900 cursor-pointer">{title}</Label>
      ...
      <Select value={value} onValueChange={onValueChange}>
        ...
        <SelectContent className="bg-white border-zinc-200 shadow-2xl rounded-2xl p-2">
          {options.map((opt) => (

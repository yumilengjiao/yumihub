...
pub fn apply_background_permission(
                ...
                while let Ok(event) = rx.recv().await {
                        match event {
                                | ConfigEvent::Basic { base } => {
                                        ...
                                        write_config!(|c| c.basic.auto_check_update =
                                                base.auto_check_update);
                                        write_config!(|c| c.basic.game_display_order =
                                                base.game_display_order);
                                ...
                                | ConfigEvent::System { sys } => {
                                        ...
                                        write_config!(|c| c.system.close_button_behavior =
                                                sys.close_button_behavior);
                                        write_config!(|c| c.system.download_concurrency =
                                                sys.download_concurrency);
                                        // 日志等级或持久化设置变更时重新应用
                                        ...
                                        let persist_changed = GLOBAL_CONFIG
                                                ...
                                                .unwrap_or(false);
                                        write_config!(
                                                |c| c.system.log_level = sys.log_level.clone()
                                        );
                                ...
                                | ConfigEvent::Interface { interface } => {
                                        ...
                                        write_config!(
                                                |c| c.interface.theme_mode = interface.theme_mode
                                        );
                                        write_config!(
                                                |c| c.interface.theme_color = interface.theme_color
                                        );
                                        write_config!(
                                                |c| c.interface.font_family = interface.font_family
                                        );
                                        write_config!(|c| c.interface.common_card_opacity =
                                                interface.common_card_opacity);
                                        set_global_background(
                                                app.clone(),
                                                interface.global_background,
                                        );
                                },
                                | ConfigEvent::Storage { storage } => {
                                        ...
                                        write_config!(
                                                |c| c.storage.gal_root_dir = storage.gal_root_dir
                                        );
                                        write_config!(|c| c.storage.allow_downloading_resources =
                                                storage.allow_downloading_resources);
                                        write_config!(
                                                |c| c.storage.auto_backup = storage.auto_backup
                                        );
                                },
